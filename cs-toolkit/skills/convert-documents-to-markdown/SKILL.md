...
5. Scanned and image-only pages need OCR, which anydoc does not do, so the document exits 3. `--ocr hosted` uploads the document to the third-party [Firecrawl Parse](https://firecrawl.dev/parse) service to OCR it. If the document may contain customer or internal CS data (PII, call recordings, VOC records, etc.), get explicit consent before using `--ocr hosted`, and prefer a local/offline OCR tool instead for sensitive or closed-network environments. Pass `--api-key` or set `FIRECRAWL_API_KEY` for higher hosted-OCR limits.
