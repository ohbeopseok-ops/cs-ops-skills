...
> LG U+ 홈CS 운영관리를 위한 AI 운영 시스템. 9개 플러그인, 32개 스킬, 20개 체인 워크플로우.  
> 품질분석 → 코칭 → 성과관리 → VOC → STT분석 → 운영기획 → 리포팅 → 판단 감사 전 영역 커버.
...
원자료를 판단 노트로 정리? → `/decision-note`  
지식베이스 모순·반복 실수 점검? → `/audit-knowledge`  
...
### 9. cs-decision-audit — 판단 감사
인터뷰 로그·VOC·회의록·투자 판단 등 원자료를 판단 추적형 노트로 구조화하고, 누적된 지식베이스의 모순·반복 실수·폐기 후보를 정기적으로 감사

이 저장소의 다른 플러그인들이 "결과물 생성"에 초점을 맞춘다면, `cs-decision-audit`은 "그 판단이 실제로 맞았는가"를 시간이 지나도 되짚을 수 있게 만드는 것이 목적이다. 원자료는 절대 수정하지 않고, 모든 판단 노트에 근거뿐 아니라 반대 근거를 함께 기록해 확증편향을 방지한다.

**Skills (3):** `source-schema` · `decision-note` · `pattern-audit`

**Commands (2):**
- `/decision-note` — 원자료를 근거·반대 근거·평가 지표를 갖춘 판단 추적형 노트로 변환
- `/audit-knowledge` — 지식베이스 전체의 모순·반복 실수·폐기 후보·출처 없는 주장·고립 노트·미해결 질문 점검

**주의:** 이 플러그인은 노트 작성 방법론(스킬)만 제공한다. 실제 상담사 인터뷰·VOC 원문 등 민감한 업무 데이터는 이 공개 저장소가 아니라 별도의 비공개 저장소나 로컬 환경에 보관해야 한다.
...
- `/decision-note 설치비 상담은 공감을 먼저 해야 한다 [근거 자료]`
- `/audit-knowledge 상담품질 도메인 노트 전체`
